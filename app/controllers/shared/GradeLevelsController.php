...
if(!in_array($role, ['admin', 'administrative'], true)){
...
AuthRole::allowOnly([$role]);
    ...
    class GradeLevelsController extends Controller{
        ...
        public function __construct($con, $role){
            parent::__construct(
        ...
        public function create($data){
            try{
                if($this->model->create($data)){
                    ...
                    header("Location: ../../../resources/views/{$this->role}/grade-level.php");
                    exit();
                }else{
    ...
    try{
        $controller = new GradeLevelsController($con, $role);
        $grade_levels = $controller->index();

        if($_SERVER['REQUEST_METHOD'] === 'POST'){
            Csrf::requireValidOnPost("../../../resources/views/{$role}/grade-level.php");
